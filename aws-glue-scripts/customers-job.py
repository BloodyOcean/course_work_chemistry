...
S3bucket_node1 = glueContext.create_dynamic_frame.from_options(
    ...
    format="json",
    connection_options={
        "paths": ["s3://pharmacy-objects/Customers/v1/"],
        "recurse": True,
    },
...
def clean_records(rec):
    ...
    # Clean up email and zip code (remove all whitespaces)
    rec["email"] = rec["email"].replace(" ", "")
    rec["zip_code"] = rec["zip_code"].replace(" ", "")
    
    # Clean up first name, last name, city, state, address (remove all whitespaces at the beginning and the end)
    rec["first_name"] = rec["first_name"].strip()
    rec["last_name"] = rec["last_name"].strip()
    rec["city"] = rec["city"].strip()
...
MySQLtable_node3 = glueContext.write_dynamic_frame.from_catalog(
    ...
    table_name="pharmacy_shop_customers",
